...
module Domain = struct
  ...
  let interval_of typ range =
    match typ with
    | T_J_U size ->
        let l, h = wsize_to_range size in
        mk_z_interval l h range
  ...
  let declare_var v range man flow =
    ...
    (* | T_J_U size -> *)
    (*   let l,h = wsize_to_range size in *)
    (*   man.exec (mk_assume (mk_in new_var (mk_constant (C_int l) range) (mk_constant (C_int h) range) range) range) flow *)
    (* | _ -> man.exec (mk_nop range) flow) *)
    (* >>% fun flow -> *)
