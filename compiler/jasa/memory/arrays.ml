...
let () =
  register_check (fun next fmt check ->
    match check with
      | CHK_J_OUT_OF_BOUNDS_ARRAY -> Format.fprintf fmt "Jasmin Array out of bounds"
      | _ -> next fmt check)



type alarm_kind += A_J_out_of_bounds_array of expr (* array *) * expr (* index *)

let () =
  register_alarm {
    check = (fun next -> function A_J_out_of_bounds_array _ -> CHK_J_OUT_OF_BOUNDS_ARRAY | a -> next a);
    compare =
      (fun next a1 a2 -> match a1, a2 with
    ...
    print =
      ...
      Format.fprintf fmt "maybe a out of bounds with %a at index %a" pp_expr arr pp_expr index
    | a -> next fmt a);
    join = (fun next -> next);
  }
...
module Domain = struct
  ...
  module J_Array = struct
    ...
    let print =
      unformat (fun fmt a -> Format.fprintf fmt "{size : %a, typ : %a}" Z.pp_print a.size pp_typ a.typ )
  ...
  let get_base typ = match typ with
    | T_J_Array (base, len) -> Utils.wsize_to_int base
    | _ -> failwith "this is not an array"

  let check_in_bounds arr access wsize ?(size = 1) index man flow range ~in_bounds =
    ...
    let index_up = match size, access with
      | 0, AAscale -> index
      | s, AAscale -> mk_binop (mk_int (size - 1) range) O_plus index range
      | s, AAdirect -> mk_binop (mk_int (scale_factor_index * size - 1) range) O_plus index range
    in
    let upper_bound = match access with
      | AAscale -> len
      | AAdirect -> scale_factor_array * len
    in
    (* Is : 0 <= index < len *)
    assume (mk_log_and ~etyp:T_bool
      ...
      ~felse:(fun fflow ->
          ...
          let alarm = mk_alarm (A_J_out_of_bounds_array (arr, index)) call_stack (erange arr) in
          Flow.raise_alarm alarm ~bottom:false man.lattice fflow |>
          Cases.empty
        )
  ...
  let exec_assign_array lval expr range man flow =
    ...
    | E_J_Laset(access,wsize,array,index) ->
      check_in_bounds (mk_var array (erange lval)) access wsize index man flow range ~in_bounds:Post.return
      |> OptionExt.return
    | E_J_Lasub(access,wsize,size,array,index) ->
      check_in_bounds (mk_var array (erange lval)) ~size access wsize index man flow range ~in_bounds:Post.return
      |> OptionExt.return
    | _ -> None
  ...
  let eval expr man flow = match ekind expr with
    | E_J_sub(access,wsize,size,array,index) ->
      ...
      check_in_bounds (mk_var array (erange expr)) ~size access wsize index man flow (erange expr) ~in_bounds:(Cases.return (mk_z_interval range_down range_up (erange expr)))
      |> OptionExt.return
    ...
    | E_J_get(access,wsize,array,index) ->
      man.eval index flow >>$? fun index flow ->
      let range_down, range_up = Utils.wsize_to_range wsize in
      check_in_bounds (mk_var array (erange expr)) access wsize index man flow (erange expr) ~in_bounds:(Cases.return (mk_z_interval range_down range_up (erange expr)))
      |> OptionExt.return

    (* E[| array[index] |] *)
    | E_J_Laset(access,wsize,array,index) ->
      check_in_bounds (mk_var array (erange expr)) access wsize index man flow (erange expr) ~in_bounds:(fun flow -> Eval.singleton expr flow)
      |> OptionExt.return
    | _ -> None
