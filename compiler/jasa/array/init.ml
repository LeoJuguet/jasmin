...
struct
  ...
  let mk_init_array ?(mode = STRONG) s = mk_attr_var ~mode s "no_init_range" T_int
  ...
  let eval expr man flow =
    ...
    | E_J_get(access,wsize, var, index) ->
      ...
      let scale_index_interval = match index_interval with
        | BOT -> BOT
      ...
      (if I.intersect_bot scale_index_interval array_no_init_interval then
        ...
        let alarm = mk_alarm (A_J_NOT_INIT_ARRAY (var, index_interval)) cs range in
    ...
    | Nb ((index_value,_) as index) when I.is_singleton index ->
      if I.intersect_bot scale_index_interval array_no_init_interval then
        ...
        | Nb (la,ha) when B.equal la index_value ->
          ...
          let (a,b) = (B.add la (B.of_int (len * (Utils.wsize_to_int wsize))), ha) in
          man.exec (mk_assign (mk_var (mk_init_array var) range) (mk_constant ~etyp:T_int (C_int_interval (a,b)) range ) range) flow
        |> OptionExt.return
        | Nb (la,ha) when B.equal ha index_value ->
          None
        | _ -> None
      else
        None
  ...
  let exec stmt man flow =
    ...
    | S_assign ({ekind = E_var(var,mode)}, {ekind = E_J_arr_init len }) ->
      ...
      man.exec (mk_assign (mk_var (mk_init_array var) range) (mk_int_interval 0 (len * 8) range) range) flow
      |> OptionExt.return
    ...
    | S_assign ({ekind = E_J_Lasub(access,wsize,len,var,index)}, t) ->
      None
    | S_add({ekind = E_var(var,mode)} as v) when is_jasmin_array_type @@ etyp v ->
  ...
  let ask : type r. ('a, r) query -> _ man -> _ flow -> ('a, r) cases option  =
    ...
    | Q_avalue({ekind = E_J_get(access,wsize,var,index)} as expr, Integer.Initialized.V_jasmin_scalar_initialized ) ->
      let open Universal.Numeric.Common in
      let range = erange expr in
      ...
      let scale_index_interval = match index_interval with
        ...
        | Nb (li,hi) -> Nb (B.mul li wsize_B, B.mul hi wsize_B)
      in
      let is_init = if I.included_bot scale_index_interval array_no_init_interval then
          Integer.Initialized.Init.not_init
      else if I.intersect_bot scale_index_interval array_no_init_interval then
          Integer.Initialized.Init.maybe
      else
          Integer.Initialized.Init.init
      in
      Cases.singleton is_init flow
      |> OptionExt.return
    | _ -> None
