...
type _ avalue_kind += V_return_value : expr list with_bot_top avalue_kind
...
module SimplifiedValue = struct
  ...
  let accept_type t = match t with T_J_Return _ -> true | _ -> false
  ...
  let subset e1 e2 =
    bot_top_included
      (fun e1 e2 -> if List.length e1 < List.length e2 then e1 < e2 else false)
      e1 e2
  ...
  let avalue : type r. r avalue_kind -> t -> r option =
   fun aval a -> match aval with V_return_value -> Some a | _ -> None
end
